...
class HiveService {
  ...
  Future<void> addItem<T>(String boxName, T item) =>
      Hive.box<T>(boxName).add(item);

  /// Appends all [items] to the typed box.
  Future<void> addItems<T>(String boxName, List<T> items) =>
      Hive.box<T>(boxName).addAll(items);

  /// Stores [item] at the given [key] inside the typed box.
  Future<void> putItem<T>(String boxName, dynamic key, T item) =>
      Hive.box<T>(boxName).put(key, item);

  /// Returns all items stored in the typed box.
  List<T> getAllItems<T>(String boxName) =>
      Hive.box<T>(boxName).values.toList();

  /// Returns the item at [key] from the typed box, or null if not found.
  T? getItem<T>(String boxName, dynamic key) =>
      Hive.box<T>(boxName).get(key);

  /// Deletes the item at [key] from the typed box.
  Future<void> removeItem<T>(String boxName, dynamic key) =>
      Hive.box<T>(boxName).delete(key);

  /// Deletes items at all [keys] from the typed box.
  Future<void> removeItems<T>(String boxName, Iterable<dynamic> keys) =>
      Hive.box<T>(boxName).deleteAll(keys);
